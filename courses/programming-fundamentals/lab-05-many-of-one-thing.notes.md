...
Лаба — **що здати** (64×32, `PLOT`, `OUTS`, `find`, тег). Фрагмент → `scratch.cpp`, збірка як у [Notes 01](lab-01-a-box-of-bytes.notes.md). Екран — команда `show` у `./build/ember` (ASCII в терміналі). **Етапи, терміни й питання на захист — у [лабі](lab-05-many-of-one-thing.md); тут вони не повторюються.** Цей файл — фрагменти, які ти запускаєш.

| | Зроби зараз | Зупинись, коли |
|---|---|---|
| **§1** | `sizeof(масив)` проти `sizeof(вказівник)` | зведення до вказівника |
| **§2** | `y * W + x` | те саме, що `m[y][x]` |
| **§3** | рядок без `'\0'` | ASan або сміття |
| **§4** | один прохід бульбашки на папері | обмін сусідів (сортування — Advanced) |
...
`dump` — і побачиш свої пікселі в шістнадцятковому вигляді.

---

## Ідея

Пам'ять ember уже масив. Цього тижня ти ним користуєшся як програміст: `i`-й елемент, сітка, текст, обміни.

Двовимірності в пам'яті немає. Є формула. А оскільки піксель — це **біт**, вона
двоповерхова:
...
`y * 8` — крок рядка. `x / 8` — який байт у рядку. `x % 8` — який біт у байті. `7 -`
...
Виставити біт — `mem[a] |= (1u << n)`, скинути — `mem[a] &= ~(1u << n)`. Це та сама
...
**Очікуй:** `12 8` на 64-бітній машині (`3*4` проти ширини вказівника). Тому в функцію передають `buf` **і** `size`. `buf[i]` це `*(buf + i)` (Lab 3).
...
сказати, де там крок рядка, де байт, а де біт — і пиши `plot`, не підглядаючи.
...
З ASan **очікуй:** звіт на другому друці, або сміття без санітайзера. Тому `OUTS` має **стелю** (max байтів або `MEM_SIZE`), не лише `'\0'`.
...
Сортування вставками: взяти наступний, зсунути більші праворуч. На вже відсортованому майже не робить обмінів — корисне питання на захисті.
...
| `HELLO\0` за `0x800` | `OUTS` зі стелею |
...
Бітова упаковка: 64×32×1 біт = 256 байтів. Байт на піксель був би 2048 — половина
