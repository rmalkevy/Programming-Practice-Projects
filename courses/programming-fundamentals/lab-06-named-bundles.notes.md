...
Лаба — **що здати** (CPU як struct, спрайти, репорти sanitizer). Фрагмент → `scratch.cpp`, збірка як у [Notes 01](lab-01-a-box-of-bytes.notes.md). Репорт ASan/LSan — це stdout/stderr термінала. **Етапи, терміни й питання на захист — у [лабі](lab-06-named-bundles.md); тут вони не повторюються.** Цей файл — фрагменти, які ви запускаєте.

| | Зроби зараз | Зупинись, коли |
|---|---|---|
| **§1** | `sizeof` struct з `char`+`int` | padding ≠ 5 |
| **§2** | `enum class` = `1` | не компілиться без приведення типу |
...
**На Linux і в WSL очікуй:** leak report — об’єкт є, імені немає.
...
Два способи здати M4 — обидва зараховуються:

- прогнати цей один файл у Linux / WSL / Docker і вставити справжній репорт;
...
чіпати. `free` немає: bump так і живе. Два `ALLOC` підряд — два шматки в дампі без
...
виглядають dangling і double-free. Купа гостя — коли лишився час.
...
| `alloc` | bump — Advanced |
| репорти санітайзера | README, зламані команди прибрати перед тегом |
