# Lab 06 — Іменовані набори: структури, переліки й купа

> «Структура — це розкладка. Перелік — це список значень. Купа — це пам'ять, час життя якої ви обрали самі й самі ж маєте скасувати.»

**Тижні:** 11–12 · **Про мову:** `struct`, `enum class`, розкладка й `sizeof`, стек проти купи, `new`/`delete`, витоки й подвійне звільнення · **Крок проєкту:** `CPU` і `Instruction` як структури, регіон купи, спрайти як записи · **Курс:** [EN](README.md) · [UK](README.uk.md) · **Попередня:** [Lab 05](lab-05-many-of-one-thing.md) · **Notes:** [теорія і досліди](lab-06-named-bundles.notes.md) · **У залізі:** [де це живе насправді](HARDWARE.md#lab-6--іменовані-набори) · **Пісочниця:** [простіший варіант](lab-06-named-bundles.sandbox.md)
...
Регістри, прапорці, `PC`, вказівник на пам'ять — з Lab 2 ви носите це в готових `struct CPU` і `struct Flags`, які читали, але не писали. Цього тижня вони стають вашими: ви з'ясовуєте, як вони лежать у пам'яті, а константи `OP_*` перетворюєте на `enum class Op`. **Запис** стає способом проєктування: `struct` — це ім'я для розкладки в пам'яті, поле за полем. `enum` — ім'я для невеликого набору цілих (опкоди, види спрайтів). Разом вони роблять так, що замість `data[i+3]` ви пишете `sprite.x`.

Друга половина лаби — **час життя**. Усе дотепер було автоматичним: `CPU cpu{}` помирає наприкінці `main`. **Купа** (`new` / `delete` або bump-алокатор **усередині** чотирьох кілобайтів `ember`) — це пам'ять, яка живе, поки ви не скажете інакше. Забудете `delete` — вас насварить LeakSanitizer. Зробите `delete` двічі (double free) — насварить ASan. А завислий вказівник — це вказівник, чий об'єкт у купі вже зник.
...
Цього тижня ви робите **хостову** половину: один `new`/`delete`, прогнаний трьома способами, щоб ASan показав вам, як насправді виглядають завислий вказівник і подвійне звільнення. Регіон `0xC00`–`0xEFF` в `ember` зарезервований під купу гостя, а зробити для неї `ALLOC` — це розділ «Якщо встигаєте»; поки що досить позначити регіон на карті пам'яті.

Правила: у кожного `new` рівно один `delete`; у кожного `new[]` — `delete[]`. Після `delete` вказівник **завис** — присвойте йому `nullptr`. Завислим вказівником не користуються. **Витік** — це об'єкт у купі, на який не лишилось жодного вказівника; **завислий вказівник** — це вказівник, у якого не лишилось об'єкта. Вони протилежні. Обидва — баги.

### 4. Про `CPU&`

У коді цієї лаби вам трапиться `step(CPU& cpu)`. `T&` — це **посилання**: вказівник, який не може бути порожнім і якому не потрібна зірочка в місці виклику. Поки що сприймайте його як «передати саму машину, а не її копію» — повний розбір і те, коли обирати `T`, `T*` чи `T&`, буде темою [Lab 7](lab-07-call-and-return.md).
...
**M1 — рефакторинг у перелік і розкладка структур.**
Константи `OP_*` з `cpu.hpp` стають `enum class Op`, і `step` перемикається по `Op`. `struct CPU` і `struct Flags` у вас із Lab 2 — надрукуйте `sizeof(CPU)` і знайдіть, де в ній вирівнювальні байти; це в README. Поведінка не змінюється: під тегом `lab-06` програми з Lab 5 мають працювати так само. Завдання в тому, щоб **дати ім'я розкладці, яка у вас і так уже була**.
...
- Константи `OP_*` замінені на `enum class Op : std::uint8_t` зі значеннями з [ISA.uk.md](ISA.uk.md).
...
- Усе вище плюс купа гостя з розділу «Якщо встигаєте»: `ALLOC` як bump-алокатор по `0xC00`–`0xEFF`, `C` виставляється, коли не влазить, дамп після двох виділень.
...
[DEFENSE.md](DEFENSE.md#lab-6--іменовані-набори). Готуйтесь не завчанням, а своїм `ember`: змініть
рядок, передбачте, перевірте.

1. Намалюйте `Sprite` у пам'яті. Де вирівнювальні байти, якщо вони є? Як ви про це дізнались?
2. `const Byte OP_ADD = 0x10` (як у вас було з Lab 2) проти `enum class Op` проти `#define ADD 0x10`. Навіщо морочитись?
