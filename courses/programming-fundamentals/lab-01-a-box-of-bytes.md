...
1. `set 0 255`, потім `get 0`, потім допишіть у диспетчер трирядкову команду `inc <addr>`: прочитати байт, додати одиницю, записати назад. `inc 0`, далі `get 0` показує `0`, а не `256` — байт загорнувся. Порівняйте це з переповненням **знакового** `int`, зібраним з UBSan ([notes §2](lab-01-a-box-of-bytes.notes.md#2-переповнення-wrap-vs-ub)): це вже невизначена поведінка, і за неї ви отримаєте звіт.
