...
|---|---|---|
| `char` / `std::uint8_t` | 1 | Байт. Символи. Комірка пам'яті `ember`. |
...
Беззнакове загортання визначене: `std::uint8_t x = 255; x = x + 1;` дає `0`. Знакове переповнення — **невизначена поведінка**: компілятор має право вважати, що його не буває, і тоді ваш цикл почне робити щось несосвітенне. У `ember` комірки пам'яті — **беззнакові байти**. Арифметика, яку ви напишете далі (Lab 2), теж загортатиметься навмисно, на `uint8_t` / `uint16_t`.

**Числа з рухомою комою** (floating point) — це експоненційний запис у двійковій системі: знак, порядок, мантиса. Більшість десяткових дробів туди не вкладаються. `0.1 + 0.2 == 0.3` дає `false`. Це не баг вашого компілятора. Не питайте у `float` через `==`, чи це «те саме значення»: або порівнюйте різницю з допуском, або (краще, і саме так у цьому курсі) тримайте гроші й пікселі в цілих.
...
### 5. Змінна — це ім'я для місця в пам'яті, а ініціалізація обов'язкова
...
Про область видимості всерйоз ітиметься в [Lab 4](lab-04-the-shape-of-control.md); поки що досить знати: ім'я живе від свого оголошення до кінця блока `{ }`, у якому воно стоїть.
...
| §4 | той самий байт читається як десяткове число, як шістнадцяткове і як символ |
...
1. `set 0 255`, потім `get 0`, потім допишіть у диспетчер трирядкову команду `inc <addr>`: прочитати байт, додати одиницю, записати назад. `inc 0`, далі `get 0` показує `0`, а не `256` — байт загорнувся. Порівняйте це з переповненням **знакового** `int`, зібраним з UBSan ([notes §2](lab-01-a-box-of-bytes.notes.md#2-переповнення-загортання-проти-ub)): це вже невизначена поведінка, і за неї ви отримаєте звіт.
...
- У README пояснено, **чому** комірка `ember` — це `std::uint8_t`, а не `int`.

### Advanced — відмінно (~16 годин)
- Усе вище плюс завдання з «Якщо встигаєте»: `set16` і відповідь про порядок байтів, доведена дампом.
...
- [ ] `dump` шістнадцятково + ASCII; `get` десятково, шістнадцятково, двійково й символом; `inc`; `quit`.
...
[DEFENSE.md](DEFENSE.md#lab-1--коробка-байтів). Готуйтесь не завчанням, а своїм `ember`: змініть
